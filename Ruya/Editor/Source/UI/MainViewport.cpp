...
{
    drawImageDescriptorSet = ImGui_ImplVulkan_AddTexture(Ruya::Engine::GetInstance().GetRenderer().GetRendererBackend()->defaultSampler, Ruya::Engine::GetInstance().GetRenderer().GetRendererBackend()->gBuffer.baseColorTexture.imageView, VK_IMAGE_LAYOUT_GENERAL);
}
